...
{
    path.setup = [&] { g_msrs[msrs::ia32_vmx_true_pinbased_ctls::addr] = 0; g_vmcs_fields[VMCS_PIN_BASED_VM_EXECUTION_CONTROLS] = 0; };
    path.throws_exception = false;
    cfg.push_back(path);

    path.setup = [&] { g_msrs[msrs::ia32_vmx_true_pinbased_ctls::addr] = 1; g_vmcs_fields[VMCS_PIN_BASED_VM_EXECUTION_CONTROLS] = 0; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("invalid pin based controls");
    cfg.push_back(path);
}
...
{
    path.setup = [&] { g_msrs[msrs::ia32_vmx_true_procbased_ctls::addr] = 0; g_vmcs_fields[VMCS_PRIMARY_PROCESSOR_BASED_VM_EXECUTION_CONTROLS] = 0; };
    path.throws_exception = false;
    cfg.push_back(path);

    path.setup = [&] { g_msrs[msrs::ia32_vmx_true_procbased_ctls::addr] = 1; g_vmcs_fields[VMCS_PRIMARY_PROCESSOR_BASED_VM_EXECUTION_CONTROLS] = 0; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("invalid proc based controls");
    cfg.push_back(path);
}
...
setup_check_control_proc_based_ctls2_reserved_properly_set_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("invalid proc based secondary controls");
    cfg.push_back(path);
}
...
setup_check_control_cr3_count_less_than_4_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("cr3 target count > 4");
    cfg.push_back(path);
}
...
setup_check_control_io_bitmap_address_bits_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("io bitmap a addr not page aligned");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("io bitmap b addr not page aligned");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_ADDRESS_OF_IO_BITMAP_B_FULL] = 0xff00000000000000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("io bitmap a addr too large");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_ADDRESS_OF_IO_BITMAP_A_FULL] = 0x1000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("io bitmap b addr too large");
    cfg.push_back(path);
...
setup_check_control_msr_bitmap_address_bits_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("msr bitmap addr not page aligned");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_ADDRESS_OF_MSR_BITMAPS_FULL] = 0xff00000000000000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("msr bitmap addr too large");
    cfg.push_back(path);
...
setup_check_control_tpr_shadow_and_virtual_apic_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("virtual apic physical addr is NULL");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_VIRTUAL_APIC_ADDRESS_FULL] = 1; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("virtual apic addr not 4k aligned");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_VIRTUAL_APIC_ADDRESS_FULL] = 0xff00000000000000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("virtual apic addr too large");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("tpr_shadow is enabled, but virtual interrupt delivery is enabled");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("bits 31:4 of the tpr threshold must be 0");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_TPR_THRESHOLD] = 0; enable_proc_ctl2(VM_EXEC_S_PROC_BASED_VIRTUALIZE_APIC_ACCESSES); };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("tpr_shadow is enabled, but virtual apic is enabled");
    cfg.push_back(path);

    path.setup = [&] { disable_proc_ctl2(VM_EXEC_S_PROC_BASED_VIRTUALIZE_APIC_ACCESSES); g_phys_to_virt_return_nullptr = true; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("virtual apic virtual addr is NULL");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("invalid TPR threshold");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("x2apic mode must be disabled if tpr shadow is disabled");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("apic register virtualization must be disabled if tpr shadow is disabled");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("virtual interrupt delivery must be disabled if tpr shadow is disabled");
    cfg.push_back(path);
...
setup_check_control_nmi_exiting_and_virtual_nmi_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("virtual NMI must be 0 if NMI exiting is 0");
    cfg.push_back(path);
...
setup_check_control_virtual_nmi_and_nmi_window_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("NMI window exiting must be 0 if virtual NMI is 0");
    cfg.push_back(path);
...
setup_check_control_virtual_apic_address_bits_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("apic access physical addr is NULL");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_APIC_ACCESS_ADDRESS_FULL] = 1; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("apic access addr not 4k aligned");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_APIC_ACCESS_ADDRESS_FULL] = 0xff00000000000000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("apic access addr too large");
    cfg.push_back(path);
...
setup_check_control_x2apic_mode_and_virtual_apic_access_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("apic accesses must be 0 if x2 apic mode is 1");
    cfg.push_back(path);
...
setup_check_control_virtual_interrupt_and_external_interrupt_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("external interrupt exiting must be 1 if virtual interrupt delivery is 1");
    cfg.push_back(path);
...
setup_check_control_process_posted_interrupt_checks_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("virtual interrupt delivery must be 1 if posted interrupts is 1");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("ack interrupt on exit must be 1 if posted interrupts is 1");
    cfg.push_back(path);

    path.setup = [&] { enable_exit_ctl(VM_EXIT_CONTROL_ACKNOWLEDGE_INTERRUPT_ON_EXIT); g_vmcs_fields[vmcs::posted_interrupt_notification_vector::addr] = 0x100; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("bits 15:8 of the notification vector must be 0 if posted interrupts is 1");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("bits 5:0 of the interrupt descriptor addr must be 0 if posted interrupts is 1");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_POSTED_INTERRUPT_DESCRIPTOR_ADDRESS_FULL] = 0xff00000000000000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("interrupt descriptor addr too large");
    cfg.push_back(path);
...
setup_check_control_vpid_checks_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("vpid cannot equal 0");
    cfg.push_back(path);
...
setup_check_control_enable_ept_checks_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("hardware does not support ept memory type: uncachable");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_EPT_POINTER_FULL] = 6; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("hardware does not support ept memory type: write-back");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_EPT_POINTER_FULL] = 3; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("unknown eptp memory type");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_EPT_POINTER_FULL] = 0xfe; g_msrs[msrs::ia32_vmx_ept_vpid_cap::addr] = IA32_VMX_EPT_VPID_CAP_WB; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("the ept walk-through length must be 1 less than 4, i.e. 3");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_EPT_POINTER_FULL] = 0x5e; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("hardware does not support dirty / accessed flags for ept");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_EPT_POINTER_FULL] = 0x9e; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("bits 11:7 and 63:48 of the eptp must be 0");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_EPT_POINTER_FULL] = 0xf00000000000001e; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("eptp must be a valid physical address");
    cfg.push_back(path);
...
setup_check_control_enable_pml_checks_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("ept must be enabled if pml is enabled");
    cfg.push_back(path);

    path.setup = [&] { disable_proc_ctl2(VM_EXEC_S_PROC_BASED_ENABLE_PML); g_vmcs_fields[VMCS_PML_ADDRESS_FULL] = 0xff00000000000000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("pml address must be a valid physical address");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_PML_ADDRESS_FULL] = 1; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("bits 11:0 of the pml address must be 0");
    cfg.push_back(path);
...
setup_check_control_unrestricted_guests_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("enable ept must be 1 if unrestricted guest is 1");
    cfg.push_back(path);
...
setup_check_control_enable_vm_functions_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("unsupported vm function control bit set");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("enable ept must be 1 if eptp switching is 1");
    cfg.push_back(path);

    path.setup = [&] { enable_proc_ctl2(VM_EXEC_S_PROC_BASED_ENABLE_EPT); g_vmcs_fields[VMCS_EPTP_LIST_ADDRESS_FULL] = 1; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("bits 11:0 must be 0 for eptp list address");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_EPTP_LIST_ADDRESS_FULL] = 0xff00000000000000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("eptp list address addr too large");
    cfg.push_back(path);
...
setup_check_control_enable_vmcs_shadowing_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("bits 11:0 must be 0 for the vmcs read bitmap address");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("bits 11:0 must be 0 for the vmcs write bitmap address");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_VMWRITE_BITMAP_ADDRESS_FULL] = 0xff00000000000000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("vmcs read bitmap address addr too large");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_VMREAD_BITMAP_ADDRESS_FULL] = 0x1000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("vmcs write bitmap address addr too large");
    cfg.push_back(path);
...
setup_check_control_enable_ept_violation_checks_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("bits 11:0 must be 0 for the vmcs virt except info address");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_VIRTUALIZATION_EXCEPTION_INFORMATION_ADDRESS_FULL] = 0xff00000000000000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("vmcs virt except info address addr too large");
    cfg.push_back(path);
...
{
    path.setup = [&] { g_vmcs_fields[VMCS_VM_EXIT_CONTROLS] = 0; g_msrs[msrs::ia32_vmx_true_exit_ctls::addr] = 1; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("invalid exit controls");
    cfg.push_back(path);

    path.setup = [&] { g_msrs[msrs::ia32_vmx_true_exit_ctls::addr] = 0; };
    path.throws_exception = false;
    cfg.push_back(path);
}
...
setup_check_control_activate_and_save_preemption_timer_must_be_0_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("save vmx preemption timer must be 0 if activate vmx preemption timer is 0");
    cfg.push_back(path);
...
setup_check_control_exit_msr_store_address_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("bits 3:0 must be 0 for the exit msr store address");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_VM_EXIT_MSR_STORE_ADDRESS_FULL] = 0xff00000000000000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("exit msr store addr too large");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_VM_EXIT_MSR_STORE_ADDRESS_FULL] = 0xfffffff0; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("end of exit msr store area too large");
    cfg.push_back(path);
...
setup_check_control_exit_msr_load_address_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("bits 3:0 must be 0 for the exit msr load address");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_VM_EXIT_MSR_LOAD_ADDRESS_FULL] = 0xff00000000000000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("exit msr load addr too large");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_VM_EXIT_MSR_LOAD_ADDRESS_FULL] = 0xfffffff0; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("end of exit msr load area too large");
    cfg.push_back(path);
...
{
    path.setup = [&] { g_vmcs_fields[VMCS_VM_ENTRY_CONTROLS] = 0; g_msrs[msrs::ia32_vmx_true_entry_ctls::addr] = 1; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("invalid entry controls");
    cfg.push_back(path);

    path.setup = [&] { g_msrs[msrs::ia32_vmx_true_entry_ctls::addr] = 0; };
    path.throws_exception = false;
    cfg.push_back(path);
}
...
setup_check_control_event_injection_type_vector_checks_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("interrupt information field type of 1 is reserved");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("interrupt information field type of 7 is reserved on this hardware");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("interrupt information field vector must be 2 if the type field is 2 (NMI)");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_VM_ENTRY_INTERRUPTION_INFORMATION_FIELD] |= 0x300; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("interrupt information field vector must be 0->31 if the type field is 3 (HE)");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("interrupt information field vector must be 0 if the type field is 7 (other)");
    cfg.push_back(path);
...
setup_check_control_event_injection_delivery_ec_checks_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("unrestricted guest must be 0 or PE must be enabled in cr0 if deliver error code bit is set");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("interrupt information field type must be 3 if deliver error code bit is set");
    cfg.push_back(path);
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("vector must indicate exception that would normally deliver an error code if deliver error code bit is set");
    cfg.push_back(path);
...
setup_check_control_event_injection_reserved_bits_checks_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("reserved bits of the interrupt info field must be 0");
    cfg.push_back(path);
}
...
setup_check_control_event_injection_ec_checks_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("bits 31:15 of the exception error code field must be 0 if deliver error code bit is set in the interrupt info field");
    cfg.push_back(path);
...
setup_check_control_event_injection_instr_length_checks_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("instruction length must be greater than zero");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_VM_ENTRY_INSTRUCTION_LENGTH] = 16; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("instruction length must be in the range of 0-15 if type is 4, 5, 6");
    cfg.push_back(path);
...
setup_check_control_entry_msr_load_address_paths(std::vector<struct control_flow_path> &cfg)
{
    ...
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("bits 3:0 must be 0 for the entry msr load address");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_VM_ENTRY_MSR_LOAD_ADDRESS_FULL] = 0xff00000000000000; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("entry msr load addr too large");
    cfg.push_back(path);

    path.setup = [&] { g_vmcs_fields[VMCS_VM_ENTRY_MSR_LOAD_ADDRESS_FULL] = 0xfffffff0; };
    path.throws_exception = true;
    path.exception = std::make_shared<std::logic_error>("end of entry msr load area too large");
    cfg.push_back(path);
