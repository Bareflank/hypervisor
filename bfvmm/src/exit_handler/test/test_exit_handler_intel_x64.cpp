...
{
    auto null_intrinsics = (intrinsics_intel_x64 *)0;
    EXPECT_EXCEPTION(std::make_unique<exit_handler_intel_x64>(null_intrinsics), bfn::invalid_argument_error);
}
...
exit_handler_intel_x64_ut::test_vm_exit_reason_unknown()
{
    ...
    auto intrinsics = mocks.Mock<intrinsics_intel_x64>();

    mocks.OnCall(intrinsics, intrinsics_intel_x64::vmread).Do(stubbed_vmread);
    mocks.OnCall(intrinsics, intrinsics_intel_x64::vmwrite).Do(stubbed_vmwrite);

    auto eh = std::make_unique<exit_handler_intel_x64>(intrinsics);
    g_exit_reason = 0xDEADBEEF;

    mocks.ExpectCall(intrinsics, intrinsics_intel_x64::stop);
...
exit_handler_intel_x64_ut::test_vm_exit_reason_cpuid()
{
    ...
    mocks.OnCall(intrinsics, intrinsics_intel_x64::vmread).Do(stubbed_vmread);
    mocks.OnCall(intrinsics, intrinsics_intel_x64::vmwrite).Do(stubbed_vmwrite);
    mocks.OnCall(intrinsics, intrinsics_intel_x64::cpuid);

    auto eh = std::make_unique<exit_handler_intel_x64>(intrinsics);
    g_exit_reason = VM_EXIT_REASON_CPUID;

    mocks.NeverCall(intrinsics, intrinsics_intel_x64::stop);
