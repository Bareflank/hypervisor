    ...
    if(CONFIG)
        foreach(c ${CONFIG})
            ...
            if(EXISTS "${c}")
                ...
                include(${c})
            elseif(EXISTS "${SOURCE_CONFIG_DIR}/${c}.cmake")
...
function(download_dependency NAME)
    set(oneVal URL URL_MD5 PREFIX)
    cmake_parse_arguments(ARG "" "${oneVal}" "" ${ARGN})

    if(NOT ARG_URL)
        message(FATAL_ERROR "Invalid URL: ${ARG_URL}")
    endif()
...
function(add_targets NAME PREFIX SOURCE_DIR)
    ...
    if(NOT PREFIX STREQUAL "none")
        add_custom_target(
            clean-${NAME}_${PREFIX}
            COMMAND ${CMAKE_COMMAND} -E remove_directory ${CMAKE_BINARY_DIR}/${NAME}/${PREFIX}
        )
        ...
        add_custom_target(
            rebuild-${NAME}_${PREFIX}
            COMMAND ${CMAKE_COMMAND} -E remove_directory ${CMAKE_BINARY_DIR}/${NAME}/${PREFIX}
            COMMAND ${CMAKE_COMMAND} --build . --target ${NAME}_${PREFIX}
        )
    endif()
    ...
    if(ENABLE_FORMAT)
        ...
        add_custom_target(
            format-${NAME}_${PREFIX}
            COMMAND ${ASTYLE_SCRIPT} ${USERSPACE_PREFIX_PATH}/bin/astyle diff ${SOURCE_DIR}
        )

        add_custom_target(
            format-${NAME}_${PREFIX}-all
            COMMAND ${ASTYLE_SCRIPT} ${USERSPACE_PREFIX_PATH}/bin/astyle all ${SOURCE_DIR}
        )
    endif()

    if(PREFIX MATCHES ${TEST_PREFIX})
...
macro(add_custom_target_info)
    if(NOT WIN32)
        ...
        if(NOT_ARG_TARGET)
            ...
            add_custom_command(
                TARGET info
                COMMAND ${CMAKE_COMMAND} -E cmake_echo_color --yellow --no-newline "    ${BF_BUILD_COMMAND}"
            ...
            right_justify("${BF_BUILD_COMMAND} ${ARG_TARGET}" 20 JUSTIFY_STR)
            add_custom_command(
                TARGET info
                COMMAND ${CMAKE_COMMAND} -E cmake_echo_color --yellow --no-newline "    ${BF_BUILD_COMMAND} ${ARG_TARGET}"
                COMMAND ${CMAKE_COMMAND} -E cmake_echo_color --red --no-newline "${JUSTIFY_STR}- "
                COMMAND ${CMAKE_COMMAND} -E cmake_echo_color --white "${ARG_COMMENT}"
            )
        endif()
...
function(add_subproject NAME PREFIX)
    ...
    if(ARG_PREFIX STREQUAL "vmm" AND NOT ENABLE_BUILD_VMM AND NOT ENABLE_BUILD_TEST)
        return()
    endif()

    if(ARG_PREFIX STREQUAL "userspace" AND NOT ENABLE_BUILD_USERSPACE)
        return()
    endif()

    if(ARG_PREFIX STREQUAL "test" AND NOT ENABLE_BUILD_TEST)
        return()
    endif()

    if(NOT ARG_SOURCE_DIR)
        set(ARG_SOURCE_DIR ${CMAKE_SOURCE_DIR}/${NAME})
    endif()
    if(NOT EXISTS ${ARG_SOURCE_DIR})
        message(FATAL_ERROR "Invalid source dir: ${ARG_SOURCE_DIR}")
    endif()

    add_targets(${NAME} ${PREFIX} ${ARG_SOURCE_DIR})
