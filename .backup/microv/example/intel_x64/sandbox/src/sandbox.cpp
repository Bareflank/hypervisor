...
void handle_cpuid_vmexit(vmexit_context &vc) noexcept
{
    ...
    switch(leaf) {
        ...
        case 1:
            ...
            break;
        // default:
            // [rax_out, rbx_out, rcx_out, rdx_out] = pal::execute_cpuid(vc, leaf, subleaf);
    }

    microv::provide_cpuid_vmexit_result(vc, rax_out, rbx_out, rcx_out, rdx_out);
    microv::advance_instruction_pointer(vc);
    microv::run(vc);
}

void init_root_vcpu(vmexit_context &vc) noexcept
{
...
{
    microv::set_init_handler(vc, init_root_vcpu);
    microv::start();
