...
public:
    ...
    ///     execution of a vcpu
    virtual void enable_interrupt_vmexit() noexcept = 0;

    /// @brief Disable vmexits for all external interrupts that arrive during
    ///     execution of a vcpu
    virtual void disable_interrupt_vmexit() noexcept = 0;
    ...
    virtual void set_interrupt_vmexit_handler(x64_vcpu_delegate func) noexcept = 0;

    /// @brief Inject an external interrupt into a vcpu
    ///
    /// @param vector The vector of the external interrupt to be injected
    virtual void inject_interrupt(uint64_t vector) noexcept = 0;
