...
bsl::errc_type vmm_init(x64_vm &root_vm, x64_platform &platform) noexcept
{
    ...
    char * bytes_1 = platform.hva_map_alloc<char>(0xF00D0000, SIZE_16KB);

    // Allocate a virtual address space of 4 MB that is mapped to the physical
    // address 0xBEEF0000, using 2 MB page granularity.
    uint32_t * bytes_2 = platform.hva_map_alloc<uint32_t>(0xBEEF0000, SIZE_4MB,
                             page_size::page_2m);
    ...
    // granularity.
    uint32_t * bytes_3 = platform.hva_map_alloc<uint32_t>(0xCAFE0000, SIZE_4KB,
